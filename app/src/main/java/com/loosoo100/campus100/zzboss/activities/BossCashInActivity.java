...
import com.loosoo100.campus100.activities.CashOutHistoryActivity;
 ...
 * 
 */
public class BossCashInActivity extends Activity implements OnClickListener {
	...
	private Handler handlerPay = new Handler() {
		...
		public void handleMessage(android.os.Message msg) {
			if (payType == 0) {
				WXPayUtil wp = new WXPayUtil(BossCashInActivity.this, "corg"
						+ seedOid, cuid, "loosoo100", "", money);
				wp.sendPay();
			} else if (payType == 1) {
				alipayUtil = new AlipayUtil(BossCashInActivity.this,
						"loosoo100", "校园100", money + "", cuid, seedOid, "corg");
				alipayUtil.pay();
				isPaying = true;
				new Thread() {
					public void run() {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.activity_boss_cashin);
	...
	public void onClick(View v) {
		...
		case R.id.btn_ok:
			if (et_money.getText().toString().trim().equals("")) {
				ToastUtil.showToast(BossCashInActivity.this,"请输入金额");
				return;
			}
			money = Integer.valueOf(et_money.getText().toString().trim());
			if (money <= 0) {
				ToastUtil.showToast(BossCashInActivity.this,"输入金额错误");
				return;
			}
			new Thread() {
				@Override
				public void run() {
					getOrderId(MyConfig.URL_POST_CASHIN_BOSS);
				}
			}.start();
			break;
		...
		httpUtils.send(HttpRequest.HttpMethod.POST, uploadHost, params,
				new RequestCallBack<String>() {
					...
					public void onSuccess(ResponseInfo<String> responseInfo) {
						...
						if (seedOid.equals("0")) {
							ToastUtil.showToast(BossCashInActivity.this,"提交失败");
						} else {
					...
					public void onFailure(HttpException arg0, String arg1) {
						...
						ToastUtil.showToast(BossCashInActivity.this,"输入金额不能为0");
					}
				});
	}
