...
import com.loosoo100.campus100.zzboss.activities.BossCommActiListActivity;
import com.loosoo100.campus100.zzboss.activities.BossCommActivityDetailFreeActivity;
import com.loosoo100.campus100.zzboss.activities.BossCommActivityDetailTogetherActivity;
...
import com.loosoo100.campus100.zzboss.activities.BossCommHeartActivity;
import com.loosoo100.campus100.zzboss.activities.BossCommListActivity;
...
import com.loosoo100.campus100.zzboss.activities.BossHomeCommActiSearchActivity;
 ...
 */
public class BossHomeFragment extends Fragment implements OnClickListener,
		OnScrollListener, OnTouchListener {

	@ViewInject(R.id.iv_empty)
	private ImageView iv_empty; // 空view
	@ViewInject(R.id.tv_location01)
	private TextView tv_location01;
	@ViewInject(R.id.tv_location02)
	private TextView tv_location02;
	...
	@ViewInject(R.id.btn_more_need)
	private Button btn_more_need;
	...
	private Handler handler = new Handler() {
		public void handleMessage(android.os.Message msg) {
			...
			if (needInfos != null && needInfos.size() > 0) {
				initNeedListView();
			}
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		view = inflater.inflate(R.layout.fragment_boss_home, container, false);
			...
			public void run() {
				...
				activityInfos = GetData
						.getBossHomeActivityInfos(MyConfig.URL_JSON_HOME_BOSS
								+ "?type=1");
				needInfos = GetData
						.getBossHomeNeedInfos(MyConfig.URL_JSON_HOME_BOSS
								+ "?type=1");
				bossInfos = GetData
	...
	public void initView() {
		...
		btn_more_need.setOnClickListener(this);
		...
		String company = activity.getSharedPreferences(UserInfoDB.USERTABLE,
				activity.MODE_PRIVATE).getString(UserInfoDB.COMPANY, "");
		tv_location01.setText(company);
		tv_location02.setText(company);
	...
	private void initNeedListView() {
		needAdapter = new BossCommSupportAdapter(activity, needInfos);
		...
		lv_need.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> parent, View view,
				...
				if (needInfos.get(position).getClassify() == 0) {
					Intent intent = new Intent(activity,
							BossCommDemandMoneyDetailActivity.class);
					intent.putExtra("did", needInfos.get(position).getId());
...
//				intent.putExtra("commLeader", false);
	...
	public void onClick(View v) {
		...
		// 搜索按钮
		case R.id.ll_search:
			intent.setClass(activity, BossHomeCommActiSearchActivity.class);
			activity.startActivity(intent);
			break;

		// 赞助社团
		case R.id.ll_comm:
			intent.setClass(activity, BossCommSupportActivity.class);
			activity.startActivity(intent);
		...
		// 社团活动
		case R.id.ll_acti:
			intent.setClass(activity, BossCommActiListActivity.class);
		...
		// 校园爱心
		case R.id.ll_heart:
			intent.setClass(activity, BossCommHeartActivity.class);
			activity.startActivity(intent);
		...
		case R.id.btn_more_comm:
			intent.setClass(activity, BossCommListActivity.class);
			activity.startActivity(intent);
			break;
		...
		case R.id.ll_community01:
			intent.setClass(activity, BossCommDetailActivity.class);
