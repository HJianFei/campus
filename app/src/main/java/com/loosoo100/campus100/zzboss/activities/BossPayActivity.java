...
/**
 * 
 * @author yang 活动最终支付activity
 */
public class BossPayActivity extends Activity implements OnClickListener {
	...
	private Handler handler = new Handler() {
		public void handleMessage(android.os.Message msg) {
			...
			if (result.equals("3")) {
				...
				ToastUtil.showToast(BossPayActivity.this,"支付成功");
				finish();
			} else if (result.equals("-5")) {
				imm.hideSoftInputFromWindow(et_pass.getWindowToken(), 0);
				ToastUtil.showToast(BossPayActivity.this,"余额不足");
				// 0.3秒后重置密码框
				...
				imm.hideSoftInputFromWindow(et_pass.getWindowToken(), 0);
				ToastUtil.showToast(BossPayActivity.this,"密码错误");
				// 0.3秒后重置密码框
				handler.postDelayed(new Runnable() {
					@Override
					public void run() {
				...
				imm.hideSoftInputFromWindow(et_pass.getWindowToken(), 0);
				ToastUtil.showToast(BossPayActivity.this,"支付失败");
				// 0.3秒后重置密码框
				handler.postDelayed(new Runnable() {
					@Override
					public void run() {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.activity_boss_pay);
	...
	public void onClick(View v) {
		...
		case R.id.ll_payOtherWay:
			Intent paychoiceIntent = new Intent(BossPayActivity.this,
					BossPayChoiceActivity.class);
