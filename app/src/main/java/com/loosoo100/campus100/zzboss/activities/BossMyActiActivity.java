 ...
 */
public class BossMyActiActivity extends Activity implements OnClickListener {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.activity_boss_myacti);
		...
		new Thread() {
			public void run() {
				list = GetData
						.getBossMyActiInfos(MyConfig.URL_JSON_ACTI_SUPPORT_BOSS
								+ cuid);
				if (!isDestroyed()) {
		...
		listView.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> parent, View view,
				...
				if (list.get(position).getClassify() == 0) {
					Intent intent = new Intent(BossMyActiActivity.this,
