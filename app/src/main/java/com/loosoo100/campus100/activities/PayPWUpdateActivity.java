...
/**
 * @author yang 支付密码修改activity
 */
public class PayPWUpdateActivity extends Activity implements OnClickListener {
    ...
    private Handler handlerReg = new Handler() {
        public void handleMessage(android.os.Message msg) {
            if (result.equals("1")) {
                ToastUtil.showToast(PayPWUpdateActivity.this, "密码修改成功");
                PayPWUpdateActivity.this.finish();
            } else {
                ToastUtil.showToast(PayPWUpdateActivity.this, "操作失败");
            }
        }
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_pay_pw_update);
        ...
        switch (v.getId()) {
            ...
            case R.id.btn_ok:
                ...
                if (phone.equals("") || message.equals("") || password.equals("")
                        || passwordSure.equals("")) {
                    ToastUtil.showToast(PayPWUpdateActivity.this, "请将信息填写完整");
                    return;
                }
                if (password.length() != 6) {
                    ToastUtil.showToast(PayPWUpdateActivity.this, "请设置6位密码长度！");
                    return;
                } else if (!password.equals(passwordSure)) {
                    ToastUtil.showToast(PayPWUpdateActivity.this, "两次密码输入不致");
                    return;
                } else if (!code.equals(MD5.md5("b0O" + message + "l1i"))) {
                    ToastUtil.showToast(PayPWUpdateActivity.this, "验证码输入错误");
                    return;
                } else {
                    new Thread() {
                        public void run() {
    ...
    private void postDataXutils(String uploadHost) {
        ...
        params.addBodyParameter("payPwd", password);
