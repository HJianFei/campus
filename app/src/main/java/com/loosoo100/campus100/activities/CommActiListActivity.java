 ...
 * @author yang 社团活动列表
 */
public class CommActiListActivity extends Activity implements OnClickListener {
		...
		new Thread() {
			...
			public void run() {
				...
				list = GetData.getBossActivityInfos(MyConfig.URL_JSON_ACTI_BOSS
						+ "?cartid=" + cartId + "&time=" + timeId + "&p="
						+ priceId + "&cityid=" + cityId + "&name="
						+ content_encoded + "&type=0" + "&sid=" + sid);
				if (!isDestroyed()) {
		...
		listview.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> parent, View view,
				...
				if (list.get(position).getClassify() == 0) {
					Intent intent = new Intent(CommActiListActivity.this,
