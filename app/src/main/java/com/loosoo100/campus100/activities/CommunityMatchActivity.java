...
/**
 * 
 * @author yang 社团活动赛事activity
 */
public class CommunityMatchActivity extends Activity implements OnClickListener {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.activity_community_match);
		...
		new Thread() {
			...
			public void run() {
				list = GetData
						.getCommunityActivityInfos(MyConfig.URL_JSON_COMMUNITY_ACTIVITY
								+ 3 + "&sid=" + sid+"&usersid="+usersid);
				if (!isDestroyed()) {
		...
		listView.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> parent, View view,
				...
				if (list.get(position).getClassify() == 0) {
					Intent intent = new Intent(CommunityMatchActivity.this,
