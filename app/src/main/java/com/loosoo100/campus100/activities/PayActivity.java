...
public class PayActivity extends Activity implements OnClickListener {
	...
	private String type = ""; // 订单的类型
								// mark小卖部,gift礼物盒子,disa借贷,acti社团活动,gcou凑一凑自己支付
...
//				ToastUtil.showToast(PayActivity.this,"支付成功");
				...
				ToastUtil.showToast(PayActivity.this,"余额不足");
				// 0.3秒后重置密码框
				...
				imm.hideSoftInputFromWindow(et_pass.getWindowToken(), 0);
				ToastUtil.showToast(PayActivity.this,"密码错误");
				// 0.3秒后重置密码框
				handler.postDelayed(new Runnable() {
					@Override
					public void run() {
	...
	public void onClick(View v) {
		...
		case R.id.ll_payOtherWay:
			Intent paychoiceIntent = new Intent(PayActivity.this,
					PayChoiceActivity.class);
			paychoiceIntent.putExtra("money", money);
			paychoiceIntent.putExtra("orderId", oid);
			paychoiceIntent.putExtra("cid", cid);
			paychoiceIntent.putExtra("type", type);
