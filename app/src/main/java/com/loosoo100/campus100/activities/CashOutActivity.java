...
public class CashOutActivity extends Activity implements OnClickListener {
	...
	private Handler handler = new Handler() {
		public void handleMessage(android.os.Message msg) {
			// 保存用户余额
			UserInfoDB.setUserInfo(CashOutActivity.this, UserInfoDB.MONEY,
					userInfo.getMoney() + "");
			ToastUtil.showToast(CashOutActivity.this,"提现申请已提交成功");
		...
		case R.id.btn_ok:
			...
			if (!isBank) {
				if (outMoney <= 0 || alipayAccount.equals("")
						|| trueName.equals("")) {
					ToastUtil.showToast(CashOutActivity.this,"请将信息填写正确");
					return;
				}
				if (outMoney > totalMoney) {
					ToastUtil.showToast(CashOutActivity.this,"金额输入错误");
					return;
				}
					...
					public void onSuccess(ResponseInfo<String> responseInfo) {
						...
						if (result.equals("1")) {
							new Thread() {
								public void run() {
									userInfo = GetData
											.getUserInfo(MyConfig.URL_JSON_USERINFO
													+ uid);
									if (!isDestroyed()) {
							...
							ToastUtil.showToast(CashOutActivity.this,"金额输入错误,请刷新后重新输入");
						} else {
							ToastUtil.showToast(CashOutActivity.this,"操作失败");
						}
					}
