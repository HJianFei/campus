...
/**
 * @author yang 最终支付activity
 */
public class PayActiActivity extends Activity implements OnClickListener {
    /**
     * 活动支付界面
     */
    ...
    private Handler handler = new Handler() {
        public void handleMessage(android.os.Message msg) {
            ...
            if (result.equals("3")) {
                ...
                ToastUtil.showToast(PayActiActivity.this, "支付成功");
                finish();
            } else if (result.equals("-5")) {
                imm.hideSoftInputFromWindow(et_pass.getWindowToken(), 0);
                ToastUtil.showToast(PayActiActivity.this, "余额不足");
                // 0.3秒后重置密码框
                ...
                imm.hideSoftInputFromWindow(et_pass.getWindowToken(), 0);
                ToastUtil.showToast(PayActiActivity.this, "密码错误");
                // 0.3秒后重置密码框
                handler.postDelayed(new Runnable() {
                    @Override
                    public void run() {
                ...
                imm.hideSoftInputFromWindow(et_pass.getWindowToken(), 0);
                ToastUtil.showToast(PayActiActivity.this, "支付失败");
                // 0.3秒后重置密码框
                handler.postDelayed(new Runnable() {
                    @Override
                    public void run() {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_pay_acti);
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            ...
            case R.id.ll_payOtherWay:
                Intent paychoiceIntent = new Intent(PayActiActivity.this,
                        PayChoiceActiActivity.class);
                paychoiceIntent.putExtra("money", money);
                paychoiceIntent.putExtra("aid", aid);
                paychoiceIntent.putExtra("cid", cid);
                paychoiceIntent.putExtra("tip", tip);
