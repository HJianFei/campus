...
/**
 * @author yang 礼物说订单详情-送出的礼物activity
 */
public class GiftOrderDetailSendActivity extends Activity implements
        OnClickListener {
    ...
    private Button btn_send; // 送出礼物按钮
    ...
    @ViewInject(R.id.send_pay)
    private LinearLayout send_pay; // 待付款的顶部布局
    @ViewInject(R.id.send_send)
    private LinearLayout send_send; // 待送出的顶部布局
    @ViewInject(R.id.send_delivery)
    private LinearLayout send_delivery; // 待发货的顶部布局
    @ViewInject(R.id.send_sure)
    private LinearLayout send_sure; // 待确认的顶部布局
    @ViewInject(R.id.send_finish)
    private LinearLayout send_finish; // 已完成的顶部布局
...
//						GiftOrderActivity.list.get(index).setIsPay(1);
//						GiftOrderActivity.adapter.notifyDataSetChanged();
                    }
                }
            }
            isLoading = false;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_gift_order_detail_send);
    ...
    private void initView() {
        ...
        if (giftOrderInfo.getIsPay() == 0) {
            hidenAll();
            send_pay.setVisibility(View.VISIBLE);
            btn_pay.setVisibility(View.VISIBLE);
            btn_cancel.setVisibility(View.VISIBLE);
        }
        // 待送出状态
        else if (giftOrderInfo.getIsPay() == 1
                && giftOrderInfo.getNeedType() == 1
                && giftOrderInfo.getUserAddress().equals("")) {
            hidenAll();
            send_send.setVisibility(View.VISIBLE);
        ...
        // 待发货状态
            ...
            hidenAll();
            send_delivery.setVisibility(View.VISIBLE);
        ...
        // 待确认状态
            ...
            hidenAll();
            send_sure.setVisibility(View.VISIBLE);
        ...
        // 已完成状态
            ...
            hidenAll();
            send_finish.setVisibility(View.VISIBLE);
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            ...
            // 送出按钮
            case R.id.btn_send:
                Intent intentSend = new Intent(this, GiftSendActivity.class);
                intentSend.putExtra("type", 3);
                intentSend.putExtra("oid", oid);
        ...
        httpUtils.send(HttpRequest.HttpMethod.GET, uploadHost,
                new RequestCallBack<String>() {
                    ...
                    public void onSuccess(ResponseInfo<String> responseInfo) {
                        ...
                        if (result.equals("1")) {
                            ToastUtil.showToast(GiftOrderDetailSendActivity.this, "操作成功");
                            if (type == 0) {
                                EventBus.getDefault().post(
                                        new MEventGiftRemove(index));
                            } else if (type == 1) {
                                EventBus.getDefault().post(
                                        new MEventGiftSendRemove(index));
                            }
                            finish();
                        } else if (result.equals("0")) {
                            ToastUtil.showToast(GiftOrderDetailSendActivity.this, "操作失败");
                        }
                    }
        ...
        httpUtils.send(HttpRequest.HttpMethod.GET, uploadHost,
                new RequestCallBack<String>() {
                    ...
                    public void onSuccess(ResponseInfo<String> responseInfo) {
                        ...
                        if (result.equals("1")) {
                            ToastUtil.showToast(GiftOrderDetailSendActivity.this, "已提醒商家尽早发货");
                        } else if (result.equals("2")) {
                            ToastUtil.showToast(GiftOrderDetailSendActivity.this, "您已提醒过了,请勿重复操作");
                        } else if (result.equals("0")) {
                            ToastUtil.showToast(GiftOrderDetailSendActivity.this, "操作失败");
                        }
                    }
        ...
        httpUtils.send(HttpRequest.HttpMethod.POST, uploadHost, params,
                new RequestCallBack<String>() {
                    ...
                    public void onSuccess(ResponseInfo<String> responseInfo) {
                        ...
                        if (result.equals("1")) {
                            ToastUtil.showToast(GiftOrderDetailSendActivity.this, "支付成功");
                            progress_update_blackbg.setVisibility(View.VISIBLE);
                            if (type == 1) {
                                EventBus.getDefault().post(
                                        new MEventGiftSendRemove(index));
                            ...
                            Intent intent = new Intent(
                                    GiftOrderDetailSendActivity.this,
                                    GiftSendActivity.class);
                            intent.putExtra("type", 3);
                            intent.putExtra("oid", oid);
                            startActivity(intent);
                        } else {
                            ToastUtil.showToast(GiftOrderDetailSendActivity.this, "支付失败");
                        }
                    }
    ...
    public void hidenAll() {
        ...
        // 隐藏所有状态的顶部布局
        send_pay.setVisibility(View.GONE);
        send_send.setVisibility(View.GONE);
        send_delivery.setVisibility(View.GONE);
        send_sure.setVisibility(View.GONE);
        send_finish.setVisibility(View.GONE);
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        if (resultCode == RESULT_OK && requestCode == 2) {
            progress_update_blackbg.setVisibility(View.VISIBLE);
            // 通知送出的礼物和全部订单更新界面
            GiftOrderSendActivity.isUpdate = true;
            GiftOrderActivity.isUpdate = true;
            new Thread() {
                public void run() {
