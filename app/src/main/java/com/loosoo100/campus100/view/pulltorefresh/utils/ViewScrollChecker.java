package com.loosoo100.campus100.view.pulltorefresh.utils;
...
public class ViewScrollChecker {
	...
	public static boolean canViewScrollVertically(View view, int direction) {
		...
		if (DEBUG_SCROLL_CHECK)
			L.e(TAG,
					"view = %s , direction is %s, canViewScrollVertically = %s",
					view, (direction > 0 ? "up" : "down"), result);
		return result;
	}
	...
	private static boolean performAbsListView(AbsListView view, int direction) {
		...
		if (childCount > 0) {
			...
			case DIRECTION_DOWN:
				...
				int listViewTop = view.getTop() + view.getPaddingTop()
						- lp.topMargin;
				if (DEBUG_SCROLL_CHECK)
					L.e(TAG, "firstItemTop=%s,listViewTop=%s", firstItemTop,
							listViewTop);
				return (view.getFirstVisiblePosition() > 0 || firstItemTop < listViewTop);
			case DIRECTION_UP:
				int lastItemBottom = view.getChildAt(childCount - 1)
						.getBottom();
				int listViewBottom = view.getBottom() - view.getPaddingBottom();
				if (DEBUG_SCROLL_CHECK)
					L.e(TAG, "lastItemBottom=%s,listViewBottom=%s",
		...
		if (DEBUG_SCROLL_CHECK)
			L.e(TAG,
					"AbsListView cannot scroll vertically or childCount is 0!!");
		return false;
	}
}
