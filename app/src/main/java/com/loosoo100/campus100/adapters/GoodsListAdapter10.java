 ...
 * @author yang 商品列表适配器
 */
public class GoodsListAdapter10 extends BaseAdapter {
	...
	public GoodsListAdapter10(Context context, List<GoodsInfo> list) {
		this.list = list;
		...
		viewHolder.rl_add.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				if (list.get(position).getStock() > list.get(position)
					...
					// 更新适配器
					GoodsListAdapter10.this.notifyDataSetChanged();
	...
	private class ViewHolder {
		...
		private RelativeLayout rl_root, rl_reduce, rl_add;
	}
