 ...
 */
public class MyCommunityActivityAdapter extends BaseAdapter {

	private List<CommunityActivityInfo> list;
	private LayoutInflater inflater;
	private Activity activity;

	public MyCommunityActivityAdapter(Context context,
	...
	public View getView(final int position, View convertView, ViewGroup parent) {
		...
		if (!list.get(position).getActiHeadShot().equals("")) {
			Glide.with(activity).load(list.get(position).getActiHeadShot())
					.asBitmap().placeholder(R.drawable.imgloading)
					.override(200, 200).into(viewHolder.raiv_picture);
		} else {
