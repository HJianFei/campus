 ...
 * @author yang 热门推荐适配器
 */
public class GiftHotAdapterHome extends BaseAdapter {

	private List<GoodsInfo> list;
	private Context context;
	private LayoutInflater inflater;

	public GiftHotAdapterHome(Context context, List<GoodsInfo> list) {
	...
	public View getView(final int position, View convertView, ViewGroup parent) {
		...
		if (convertView == null) {
			...
			convertView = inflater.inflate(R.layout.item_home_down, null);
		...
		viewHolder.tv_collect.setText(""
				+ list.get(position).getCollection());
		viewHolder.tv_recommend.setText("" + list.get(position).getGoodsName());
		viewHolder.iv_recommendIcon.setImageBitmap(list.get(position).getGoodsIcon());
		...
		viewHolder.ll_collect.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				if (list.get(position).isCollected()){
					list.get(position).setCollection(list.get(position).getCollection()-1);
					list.get(position).setCollected(false);
				}else {
					list.get(position).setCollection(list.get(position).getCollection()+1);
					list.get(position).setCollected(true);
				}
				GiftHotAdapterHome.this.notifyDataSetChanged();
			}
		});
